...
function orderManage() {
  ...
  const getPagingList_ = async (value) => {
    ...
    let res = await requestw({
      url: '/web/export/getCustomerTradeExportListPaging',
      data: {
        ...value,
      },
    })
  ...
  async function onFinish(values) {
    ...
    let res = await requestw({
      url: '/web/customerTrade/getCustomerTradeCountList',
      data: {
        ...values,
      },
    })
            ...
            <Form style={{ border: '1px solid #FEFFFE' }} form={form} name="basic" onFinish={onFinish}>
              <div>
                <div className="flexjss" style={{ border: '1px solid #FEFFFE', marginTop: '23px' }}>
                  <Form.Item name="customerPhone" style={{ width: '220px', marginRight: '10px' }}>
                    <Input placeholder="客户手机号" style={{ width: '220px' }} />
                  </Form.Item>
                  <Form.Item name="times" style={{ width: '220px', marginRight: '10px' }} initialValue={[moment(), moment()]}>
                    <DatePicker.RangePicker format="YYYY-MM-DD" />
                  </Form.Item>
                  <Form.Item name="distributeName" style={{ width: '220px', marginRight: '10px' }}>
                    <Input placeholder="合伙人分身名称" style={{ width: '220px' }} />
                  </Form.Item>
                  <Form.Item name="developPhone" style={{ width: '220px', marginRight: '10px' }}>
                    <Input placeholder="推广人手机号" style={{ width: '220px' }} />
                  </Form.Item>

                  <Form.Item name="sourceMode" style={{ width: 220, marginRight: '10px' }}>
                    <Select showArrow={true} placeholder="来源方式" allowClear={true}>
                      {sourceModeList.map((r, index) => (
              ...
              <div className="rr20">
                ...
                {orderdata && orderdata.tradeTypeName && orderdata.tradeTypeName !== '主订单' ? (
                  orderdata.tradeStatus === '50' || orderdata.tradeStatus === '10' ? (
                    <Button style={{ width: '100px', borderRadius: '4px' }} onClick={calceorder} type="primary">
                      取消订单
                    </Button>
                  ) : (
                    ''
