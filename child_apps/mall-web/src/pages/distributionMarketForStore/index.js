...
function Index(props) {
  ...
  const doctorCode = location.query?.doctorCode
  ...
  const getTableList = async () => {
    ...
    setSpinShow(true)
    requestw({
      url: '/web/system/drugstore/queryGoodsPage',
      data: data,
  ...
  // 添加到 药房
  async function handleAddToStore(item, index) {
    const postData = {
      doctorCode,
    ...
    setSpinShow(true)
    return requestw({
      url: '/web/system/drugstore/createDrugstore',
      data: postData,
      isNeedCheckResponse: true,
      errMsg: true,
    })
      ...
      .then((data) => {
        ...
        setTableData((list) => {
          ...
          listTemp[index].drugstoreCount = 1
          return listTemp
        })
      })
  }
      ...
      <Spin spinning={spinShow}>
        <div className="global_table_box">
          ...
          {tableData && tableData.length ? (
            <>
              <GoodItem items={tableData} goodClick={handleAddToStore} style={{ margin: '0 20px', width: '100%' }} />
              <div
                style={{
                  width: '100%',
                  margin: '20px 0',
