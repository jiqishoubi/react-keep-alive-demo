...
import { Button, Col, DatePicker, Form, Input, message, Modal, Radio, Row, Select, Table } from 'antd'
import { CopyOutlined, DownloadOutlined, DownOutlined, ExclamationCircleOutlined, UpOutlined } from '@ant-design/icons'
...
import './orderManage/index_localName.less'
...
function orderManage() {
      ...
      render: (e) => {
        return (
          <div>
            ...
            {e.tradeStatusName === '待发货' &&
              e.tradeType !== 'BLD_MALL' &&
              e.tradeTypeName !== '主订单' &&
              e.tradeMode != 'INTERNATION' &&
              e.ifCompanyExpress === true &&
              haveCtrlElementRight('ddgl-fahuo-btn') && (
                <a
                  onClick={(event) => {
                    event.stopPropagation()
                    orderOnFinishow(e)
            ...
            {e.tradeStatusName === '已发货' && e.tradeTypeName !== '主订单' && e.expressStatus === '3' && e.ifCompanyExpress === true && haveCtrlElementRight('ddgl-hswl-btn') && (
              <a
                onClick={(event) => {
                  event.stopPropagation()
            ...
            {e.tradeStatus === '831' && e.ifCompanyExpress === true && e.tradeType !== 'BLD_MALL' && haveCtrlElementRight('ddgl-tksh-btn') && (
              <a
                onClick={(event) => {
                  event.stopPropagation()
                  refundReview(e)
  ...
  const onExpand = (e, f) => {
    const id = f.id
    let expandDate = JSON.parse(JSON.stringify(expandedRow))
    let num = expandedRow.findIndex((r) => r === id)
    num === -1 ? expandDate.push(f.id) : expandDate.splice(num, 1)
    setExpandedRow(expandDate)
  }

  const fetchOnChange = (code) => {
    getSelect1(code)
    form.setFieldsValue({
      firstGroupCode: '',
      secondGroupCode: '',
  ...
  return (
    <div>
      {orderlogistics ? <Logistics show={true} onlogistics={onlogistics} orderValue={orderdata} /> : ''}
                ...
                <Row gutter={10} justify="start">
                  ...
                  <Col span={3}>
                    <Form.Item name="startDate" initialValue={moment().add(-30, 'days')} style={{ marginBottom: 10 }}>
                      <DatePicker style={{ width: '100%' }} format={'YYYY-MM-DD'} placeholder="请选择开始日期" allowClear={true} />
                    </Form.Item>
                  </Col>
                  <Col span={3}>
                    <Form.Item name="endDate" initialValue={moment()} style={{ marginBottom: 10 }}>
                      <DatePicker style={{ width: '100%' }} format={'YYYY-MM-DD'} placeholder="请选择结束日期" allowClear={true} />
                    </Form.Item>
                  </Col>
                  {/* no类型 */}
                  <Col span={3}>
                  ...
                  <Col span={3}>
                    <Form.Item name="supplierOrgCode" style={{ marginBottom: 10 }}>
                      <FetchSelect
                        placeholder="供货商企业名称"
                  ...
                  {haveCtrlElementRight('ddgl-piliangfahuo-btn') && getOrgKind().isCompany && (
                    <Col>
        ...
        <div className="positionre">
          <div className="fontMb">
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              {orderdata && orderdata.tradeTypeName && orderdata.tradeTypeName !== '主订单' && orderdata.tradeType !== 'BLD_MALL' ? (
