  ...
  <SearchForm ref="searchFormRef" :formModel="searchController.formModel" @search="tableController.search" :searchLoading="tableController.loading">
    <template #form>
      <el-form-item prop="departName">
        <el-input placeholder="科室名称" v-model="searchController.formModel.departName" clearable></el-input>
      </el-form-item>
...
export default {
  setup() {
    ...
    // table
    const columns = [
      { label: '科室', prop: 'departName', width: 170 },
      { label: '诊疗科目', prop: 'departClassFullName' },
      { label: '科室简介', prop: 'departDesc' },
    ...
    const tableController = useTableController({
      ...
      ajax: ({ page, pageSize }) => {
        ...
        return request({
          url: '/web/system/depart/getDepartList',
          data: postData,
