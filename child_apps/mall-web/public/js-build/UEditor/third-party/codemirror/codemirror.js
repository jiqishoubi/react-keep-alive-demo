  ...
  function CodeMirror(place, givenOptions) {
    ...
    for (var opt in defaults)
      if (defaults.hasOwnProperty(opt))
        options[opt] = (givenOptions && givenOptions.hasOwnProperty(opt) ? givenOptions : defaults)[
          opt
        ];
    ...
    function updateDisplay(changes, suppressCallback) {
      ...
      var different =
        from != showingFrom || to != showingTo || lastSizeC != scroller.clientHeight + th;
    ...
    function updateCursor() {
      ...
      if (posEq(sel.from, sel.to)) {
        ...
        cursor.style.left =
          (options.lineWrapping ? Math.min(pos.x, lineSpace.offsetWidth) : pos.x) + 'px';
    ...
    function charFromX(line, x) {
      ...
      function getX(len) {
        measure.innerHTML =
          '<pre><span>' + lineObj.getHTML(null, null, false, tabText, len) + '</span></pre>';
    ...
    function measureLine(line, ch) {
      ...
      if (options.lineWrapping) {
        ...
        extra = htmlEscape(
          line.text.slice(ch + 1, end < 0 ? line.text.length : end + (ie ? 5 : 0)),
        );
      }
      measure.innerHTML =
        '<pre>' +
    ...
    function posFromMouse(e, liberal) {
      ...
      if (
        !liberal &&
        (x - offW.left > scroller.clientWidth || y - offW.top > scroller.clientHeight)
      )
        return null;
    ...
    function matchBrackets(autoclear) {
      var head = sel.inverted ? sel.from : sel.to,
        ...
        pos = head.ch - 1;
      var match =
        (pos >= 0 && matching[line.text.charAt(pos)]) || matching[line.text.charAt(++pos)];
      if (!match) return;
      ...
      function scan(line, from, to) {
        ...
        for (
          ...
          for (
            var j = forward ? 0 : text.length - 1, te = forward ? text.length : -1;
            j != te;
            j += d, pos += d
          ) {
      ...
      for (
        var i = head.line, e = forward ? Math.min(i + 100, doc.size) : Math.max(-1, i - 100);
        i != e;
        i += d
      ) {
        var line = getLine(i),
        ...
        doc.iter(i, doc.size, function(line) {
          ...
          if (compare) {
            ...
            if (changed !== false || !hadState) unchanged = 0;
              ...
              ++unchanged > 3 &&
              (!mode.indent || mode.indent(hadState, '') == mode.indent(state, ''))
            )
              return true;
          }
          ++i;
  ...
  return {
    ...
    indent: function(state, textAfter) {
      ...
      if (type == 'vardef') return lexical.indented + 4;
        ...
        return (
          lexical.indented + (/^(?:case|default)\b/.test(textAfter) ? indentUnit : 2 * indentUnit)
        );
