...
import { queryListAjax, deleteTemplateDataAjax, copyCompanyUiTemplateAjax } from './services'
...
const Index = (props) => {
  ...
  // 点击复制
  async function copyTemplate(record) {
    const res = await copyCompanyUiTemplateAjax({
            ...
            tableData.map((template, index) => {
              ...
              return (
                <div key={index} className={styles.template_item}>
                  <div className={styles.template_item_title}>
                    {template.templateDataName || ''}
                    {template.ifDefault == '1' && <span className={styles.defaultText}>默认</span>}
                  </div>
