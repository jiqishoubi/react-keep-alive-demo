...
export default {
  ...
  setup() {
    ...
    const { setExportPostData, exportStream } = useExport('/web/system/doctororder/exportDoctorOrderList')

    // table
    const columns = [
      { label: '下单时间', prop: 'orderDateStr' },
      { label: '订单编号', prop: 'orderNo' },
      { label: '问诊类型', prop: 'orderTypeName' },
      { label: '医生', prop: 'doctorName', width: 140 },
      // { label: '医生集团', prop: 'orgName', width: 260 },
      { label: '患者姓名', prop: 'patientName' },
      { label: '患者手机号', prop: 'patientPhoneNumber', width: 130 },
      { label: '金额(元)', prop: 'doctorFeeStr', width: 150 },
      ...
      { label: '完成时间', prop: 'finishDateStr' },
      {
        ...
        width: 150,
        render: (_, record) => {
          // 取消订单 (注:仅当 orderStatus = 0(诊疗单提交), 10(待支付), 40 待接诊, 50(咨询中)时,可操作)
          return (
            <div>
              <CustomLink
                onClick={() => {
              ...
              {['0', '10', '40', '50'].includes(record.orderStatus) && (
                <CustomLink
                  onClick={() => {
              ...
              {['40', '50'].includes(record.orderStatus) && (
                <CustomLink
                  onClick={() => {
    ...
    const tableController = useTableController({
      ...
      ajax: ({ page, pageSize }) => {
        ...
        return request({
          url: '/web/system/doctororder/getDoctorOrderList',
          data: postData,
    ...
    const useLogDrawerController = useLogDrawer({
      ...
      api: '/web/system/doctororder/getLogDoctorOrderOperList',
    ...
    function handleDetail(record) {
      router.push(`/web/system/doctororder/ordermgr/detail?orderNo=${record.orderNo}`)
    }
    function handleCancelOrder(record) {
      promptConfirm({ label: '取消说明' }, (value) => {
        return request({
          url: '/web/system/doctororder/cancelDoctorOrder',
          data: {
    ...
    function handleFinishOrder(record) {
      promptConfirm({ label: '完成说明' }, (value) => {
        return request({
          url: '/web/system/doctororder/finishDoctorOrder',
          data: {
            orderNo: record.orderNo,
            resultNote: value ?? '',
          },
