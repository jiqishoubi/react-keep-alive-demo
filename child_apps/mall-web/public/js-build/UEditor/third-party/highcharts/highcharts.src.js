...
(function() {
  ...
  function normalizeTimeTickInterval(tickInterval, unitsOption) {
    ...
    for (i = 0; i < units.length; i++) {
      ...
      if (units[i + 1]) {
        ...
        var lessThan =
          (interval * multiples[multiples.length - 1] + timeUnits[units[i + 1][0]]) / 2;
  ...
  function getTimeTicks(normalizedInterval, min, max, startOfWeek) {
    ...
    if (defined(min)) {
      ...
      if (interval >= timeUnits[SECOND]) {
        ...
        minDate.setSeconds(
          interval >= timeUnits[MINUTE] ? 0 : count * mathFloor(minDate.getSeconds() / count),
        );
      }
      ...
      if (interval >= timeUnits[MINUTE]) {
        // minute
        minDate[setMinutes](
          interval >= timeUnits[HOUR] ? 0 : count * mathFloor(minDate[getMinutes]() / count),
        );
      }
      ...
      if (interval >= timeUnits[HOUR]) {
        // hour
        minDate[setHours](
          interval >= timeUnits[DAY] ? 0 : count * mathFloor(minDate[getHours]() / count),
        );
      }
      ...
      if (interval >= timeUnits[DAY]) {
        // day
        minDate[setDate](
          interval >= timeUnits[MONTH] ? 1 : count * mathFloor(minDate[getDate]() / count),
        );
      }
      ...
      if (interval >= timeUnits[MONTH]) {
        // month
        minDate[setMonth](
          interval >= timeUnits[YEAR] ? 0 : count * mathFloor(minDate[getMonth]() / count),
        );
  ...
  SVGElement.prototype = {
    ...
    init: function(renderer, nodeName) {
      var wrapper = this;
      wrapper.element =
        nodeName === 'span' ? createElement(nodeName) : doc.createElementNS(SVG_NS, nodeName);
            ...
            if (shadows && /^(width|height|visibility|x|y|d|transform|cx|cy|r)$/.test(key)) {
              ...
              while (i--) {
                attr(
                  shadows[i],
                  key,
                  key === 'height' ? mathMax(value - (shadows[i].cutHeight || 0), 0) : value,
                );
              }
            }
	...
	*/
    ...
    crisp: function(strokeWidth, x, y, width, height) {
      ...
      strokeWidth =
        strokeWidth || wrapper.strokeWidth || (wrapper.attr && wrapper.attr('stroke-width')) || 0;
    ...
    htmlUpdateTransform: function() {
      ...
      if (elem.tagName === 'SPAN') {
        ...
        if (currentTextTransform !== wrapper.cTT) {
          ...
          yCorr -=
            costheta * baseline * (rotation ? (quad ? alignCorrection : 1 - alignCorrection) : 1);
    ...
    updateTransform: function() {
      ...
      if (inverted) {
        ...
        // text rotation
        transform.push(
          'rotate(' + rotation + ' ' + (wrapper.x || 0) + ' ' + (wrapper.y || 0) + ')',
        );
      }
  ...
  SVGRenderer.prototype = {
                ...
                attr(
                  ...
                  textLineHeight ||
                    renderer.fontMetrics(
                      /px$/.test(tspan.style.fontSize) ? tspan.style.fontSize : textStyles.fontSize,
                    ).h,
    ...
    label: function(str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
      ...
      function updateBoxSize() {
        ...
        bBox =
          (width === undefined || height === undefined || wrapper.styles.textAlign) &&
          text.getBBox();
        ...
        if (isString(hash)) {
          ...
          for (key in hash) {
            ...
            if (result !== false && value !== null) {
              ...
              if (
                symbolName &&
                /^(x|y|r|start|end|width|height|innerR|anchorX|anchorY)/.test(key)
              ) {
                // if one of the symbol size affecting parameters are changed,
                // check all the others only once for each call to an element's
                // .attr() method
  ...
  Tick.prototype = {
    ...
    getMarkPath: function(x, y, tickLength, tickWidth, horiz, renderer) {
      return renderer.crispLine(
        [M, x, y, L, x + (horiz ? 0 : -tickLength), y + (horiz ? tickLength : 0)],
        tickWidth,
      );
    },
  ...
  StackItem.prototype = {
    ...
    setOffset: function(xOffset, xWidth) {
      ...
      if (label) {
        ...
        label.attr({
          visibility:
            this.options.crop === false || chart.isInsidePlot(alignAttr.x, alignAttr.y)
              ? hasSVG
                ? 'inherit'
                : VISIBLE
              : HIDDEN,
  ...
  Axis.prototype = {
    ...
    update: function(newOptions, redraw) {
      ...
      newOptions = chart.options[this.xOrY + 'Axis'][this.options.index] = merge(
        this.userOptions,
        newOptions,
      );
    ...
    toPixels: function(value, paneCoordinates) {
      return (
        this.translate(value, false, !this.horiz, null, true) + (paneCoordinates ? 0 : this.pos)
      );
    },
    ...
    setAxisTranslation: function(saveOld) {
      ...
      if (axis.isXAxis) {
        ...
        ordinalCorrection =
          axis.ordinalSlope && closestPointRange ? axis.ordinalSlope / closestPointRange : 1; // #988, #1853
      ...
      if (
        ...
        if (calculatedTickAmount < tickAmount) {
          while (tickPositions.length < tickAmount) {
            tickPositions.push(
              correctFloat(tickPositions[tickPositions.length - 1] + axis.tickInterval),
            );
          }
    ...
    getTitlePosition: function() {
      ...
      return {
        x: horiz
          ? alongAxis
          : offAxis + (opposite ? this.width : 0) + offset + (axisTitleOptions.x || 0), // x
        y: horiz
    ...
    render: function() {
      ...
      if (hasData || isLinked) {
        ...
        if (!axis._addedPlotLB) {
          // only first time
          each((options.plotLines || []).concat(options.plotBands || []), function(
            plotLineOptions,
          ) {
    ...
    destroy: function(keepEvents) {
      ...
      each(
        ['stackTotalGroup', 'axisLine', 'axisGroup', 'gridGroup', 'labelGroup', 'axisTitle'],
        function(prop) {
          if (axis[prop]) {
    ...
    getAnchor: function(points, mouseEvent) {
      ...
      if (!ret) {
        each(points, function(point) {
          ...
          plotX += point.plotX;
          plotY +=
            (point.plotLow ? (point.plotLow + point.plotHigh) / 2 : point.plotY) +
  ...
  Pointer.prototype = {
    ...
    pinchTranslateDirection: function(
      ...
      transform['translate' + XY] =
        transformScale * plotLeftTop + (touch0Now - transformScale * touch0Start);
    },
    ...
    pinch: function(e) {
      var self = this,
        ...
        fireClickEvent =
          touchesLength === 1 &&
          ((self.inClass(e.target, PREFIX + 'tracker') && chart.runTrackerClick) ||
            chart.runChartClick),
        clip = {};
            ...
            if (axis.zoomEnabled) {
              var horiz = axis.horiz,
                ...
                selectionMax = axis.toValue(
                  horiz ? selectionLeft + selectionBox.width : selectionTop + selectionBox.height,
                );
  ...
  Legend.prototype = {
    ...
    render: function() {
      ...
      each(chart.series, function(serie) {
        ...
        allItems = allItems.concat(
          serie.legendItems || (seriesOptions.legendType === 'point' ? serie.data : serie),
        );
      });
    ...
    scroll: function(scrollBy, animation) {
      ...
      if (currentPage > 0) {
        ...
        scrollOffset =
          -mathMin(clipHeight * (currentPage - 1), this.fullHeight - clipHeight + padding) + 1;
  ...
  Chart.prototype = {
    ...
    getStacks: function() {
      ...
      each(chart.series, function(series) {
        if (
          ...
          (series.visible === true || chart.options.chart.ignoreHiddenSeries === false)
        ) {
    ...
    pan: function(e, panning) {
      ...
      each(panning === 'xy' ? [1, 0] : [1], function(isX) {
        ...
        var mousePos = e[isX ? 'chartX' : 'chartY'],
          ...
          newMax =
            axis.toValue(startPos + chart[isX ? 'plotWidth' : 'plotHeight'] - mousePos, true) -
            halfPointRange;
    ...
    getContainer: function() {
      ...
      chart.container = container = createElement(
        ...
        {
          className:
            PREFIX + 'container' + (optionsChart.className ? ' ' + optionsChart.className : ''),
          id: containerId,
        },
        extend(
          {
      ...
      if (plotBackgroundImage) {
        if (!plotBGImage) {
          chart.plotBGImage = renderer
            .image(plotBackgroundImage, plotLeft, plotTop, plotWidth, plotHeight)
            .add();
        } else {
  ...
  Series.prototype = {
    ...
    drawLegendSymbol: function(legend) {
      var options = this.options,
        ...
        verticalCenter =
          legend.baseline -
          mathRound(renderer.fontMetrics(legendOptions.itemStyle.fontSize).b * 0.3),
        attr;
    ...
    setStackedPoints: function() {
      if (
        ...
        (this.visible !== true && this.chart.options.chart.ignoreHiddenSeries !== false)
      ) {
        return;
      }
    ...
    translate: function() {
      ...
      for (i = 0; i < dataLength; i++) {
        var point = points[i],
          ...
          yBottom = point.low,
          stack =
            yAxis.stacks[(series.negStacks && yValue < threshold ? '-' : '') + series.stackKey],
          pointStack,
          stackValues;
        ...
        // some API data
        point.category =
          categories && categories[point.x] !== UNDEFINED ? categories[point.x] : point.x;
      }
            ...
            ? mathMin(
                mathMax(
                  ...
                  mathFloor(
                    // #2070
                    (point.clientX +
                      (nextPoint ? nextPoint.wrappedClientX || nextPoint.clientX : axisLength)) /
                      2,
                  ),
                ),
                axisLength,
  ...
  var SplineSeries = extendClass(Series, {
    ...
    getPointSpline: function(segment, point, i) {
      ...
      if (lastPoint && nextPoint) {
        ...
        correction =
          ((rightContY - leftContY) * (rightContX - plotX)) / (rightContX - leftContX) +
          plotY -
          rightContY;
    ...
    getColumnMetrics: function() {
      ...
      var categoryWidth = mathMin(
          mathAbs(xAxis.transA) *
            (xAxis.ordinalSlope || options.pointRange || xAxis.closestPointRange || 1),
          xAxis.len, // #1535
        ),
  ...
  var PieSeries = {
    ...
    translate: function(positions) {
      ...
      series.getX = function(y, left) {
        ...
        return (
          positions[0] + (left ? -1 : 1) * (mathCos(angle) * (positions[2] / 2 + labelDistance))
        );
      };
