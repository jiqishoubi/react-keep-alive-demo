...
import { Card, Form, Input, Select, Row, Col, Button, Image, Modal, Descriptions, Spin, Divider } from 'antd'
...
const Index = (props, ref) => {
  ...
  const loadParamsDeal = (info) => {
    ...
    info.showList = [
      ...
      { type: 'common', label: '是否包邮', value: info?.ifShip ? '是' : '否' },
    ...
    if (info?.wechatConfigDTO) {
      ...
      info.showList.push(
        ...[
          ...
          { type: 'common', label: '启动页图片', value: info?.startPageImg },
          { type: 'common', label: '小程序热词', value: info?.hotWord },
